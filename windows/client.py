...
<script>
  ...
  function pollCommands() {{
    ...
    fetch('http://127.0.0.1:' + statePort + '/cmd')
      ...
      .then(function(d) {{
        if (d.seek !== undefined && v.duration) {{
          v.currentTime = Math.min(Math.max(0, d.seek), v.duration - 0.1);
...
def show_video_overlay(video_path: str, lock: bool, duration: int,
    ...
    def _do_seek_5():
        ...
        new_t = min(cur + 5.0, dur - 0.1) if dur > 0 else cur + 5.0
        _pending_seek[0] = new_t
